 ...
 * (grow@mail.valuesuppliers.co) to protect the main domain.
 *
 * Rate limited: max 100/batch via API, use the CLI script for higher volume.
 */

const OUTREACH_FROM = 'Value Suppliers <grow@mail.valuesuppliers.co>';
const UNSUBSCRIBE_URL = 'https://valuesuppliers.co/api/unsubscribe';
...
valuesuppliers.co/wholesale if you ever want to check pricing.
...
admin@valuesuppliersdirect.com`,
  ...
  return {
    ...
    html: `<pre style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;font-size:14px;line-height:1.6;color:#1a1a1a;white-space:pre-wrap;">${t.text.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</pre><p style="font-size:11px;color:#999;margin-top:24px;"><a href="${UNSUBSCRIBE_URL}?email=${encodeURIComponent(lead.email)}" style="color:#999;">Unsubscribe</a> | Value Suppliers, 1401 N Clovis Ave STE #103, Clovis, CA 93727</p>`,
  };
}
