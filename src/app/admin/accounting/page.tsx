...
export default function AccountingLanding() {
  ...
  const statCards = summary
    ? [
        { label: 'Total Revenue', value: fmt(summary.revenue), color: '#1B3A2D' },
        { label: 'Total Orders', value: summary.order_count.toLocaleString(), color: '#1C1C1C' },
        { label: 'Avg Order Value', value: fmt(summary.avg_order_value), color: '#C8922A' },
  ...
  return (
    <div style={{ maxWidth: 1024, margin: '0 auto', padding: '32px 24px 64px' }}>
      ...
      {loading ? (
        ...
        <div style={{ padding: 24, background: '#FFF8F0', border: '1px solid #E4E1DB', borderRadius: 12, color: '#C8922A', fontSize: '0.85rem', marginBottom: 24 }}>
          {error}
        </div>
      ) : (
