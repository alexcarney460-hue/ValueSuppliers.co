    ...
    <div style={{ maxWidth: 1024, margin: '0 auto', padding: '32px 24px 64px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 16, marginBottom: 28 }}>
        ...
        <div style={CARD}>
          ...
          <div style={{ ...BIG_NUM, color: 'var(--color-amber, #C8922A)' }}>{fmtCurrency(data.total_amount)}</div>
        </div>
      </div>
              ...
              <tbody>
                {data.refunds.map((r, i) => (
                  <tr key={`${r.order_id}-${i}`} style={{ borderBottom: ROW_BORDER }}>
                    ...
                    <td style={{ ...TD_STYLE, textAlign: 'right', fontWeight: 700, color: 'var(--color-amber, #C8922A)' }}>{fmtCurrency(r.amount)}</td>
