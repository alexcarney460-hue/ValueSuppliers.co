...
export default function SubscriptionManager({ email }: Props) {
  ...
  if (loading) {
    return (
      ...
      >
        <Loader2 size={16} style={{ animation: 'vs-spin 0.7s linear infinite' }} color="var(--color-warm-gray)" />
  ...
  const statusStyles: Record<string, { bg: string; color: string; label: string }> = {
    active: { bg: '#EDF7F0', color: 'var(--color-muted-green)', label: 'Active' },
    paused: { bg: '#FFF8EC', color: 'var(--color-amber)', label: 'Paused' },
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20 }}>
        <RefreshCw size={16} color="var(--color-muted-green)" />
        <p
                ...
                {items.map((item, idx) => (
                  ...
                  >
                    {item.purchaseUnit === 'case' ? (
                      <Package size={13} color="var(--color-forest)" />
                    ) : (
              ...
              {sub.status !== 'cancelled' && (
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {sub.status === 'active' && (
                    ...
                    >
                      {isLoading ? (
                        <Loader2 size={11} style={{ animation: 'vs-spin 0.7s linear infinite' }} />
                      ) : (
                        <Pause size={11} />
                      )}
                      Pause
